"""Day29 Movies medium_20neg_2000 preflight and status reports.

This file intentionally does not call any API. It only validates local inputs
and summarizes existing prediction outputs.
"""
...
CONFIG_PATH = Path("configs/exp/movies_deepseek_relevance_evidence_medium_20neg_2000.yaml")
SUMMARY_DIR = Path("output-repaired/summary")
EXP_DIR = Path("output-repaired/movies_deepseek_relevance_evidence_medium_20neg_2000")
...
def _split_stats(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "schema_ok": not any(v for k, v in missing.items() if k != "candidate_title_optional"),
    }
...
def preflight() -> None:
    ...
    checks = {
        ...
        "valid_rows_ok": valid["rows"] == 42000,
        "test_rows_ok": test["rows"] == 42000,
        "valid_candidate_pool_ok": valid["candidate_pool_size_min"] == 21 and valid["candidate_pool_size_max"] == 21,
        "test_candidate_pool_ok": test["candidate_pool_size_min"] == 21 and test["candidate_pool_size_max"] == 21,
        "valid_schema_ok": valid["schema_ok"],
        "test_schema_ok": test["schema_ok"],
        "output_dir_isolated": "movies_deepseek_relevance_evidence_medium_20neg_2000" in str(EXP_DIR),
    }
    ...
    lines.extend(
        [
            ...
            "## Decision",
            "",
            "Preflight passed. Tiny API smoke can start." if passed else "Preflight failed. Do not start API inference.",
        ]
    )
    (SUMMARY_DIR / "day29_movies_medium20_2000_preflight_check.md").write_text("\n".join(lines), encoding="utf-8")
...
def smoke_report() -> None:
    ...
    lines = ["# Day29 Movies medium_20neg_2000 Smoke Report", ""]
    passed_all = True
    for split, path in paths.items():
        ...
        lines.extend(
            [
                ...
                f"- path: `{path}`",
                f"- rows: `{len(rows)}`",
                f"- parse_success_rate: `{rate:.4f}`",
                f"- passed: `{passed}`",
                f"- field_counts: `{field_counts}`",
                "",
            ]
        )
    ...
    (SUMMARY_DIR / "day29_movies_medium20_2000_smoke_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def runtime_monitor() -> None:
    ...
    (SUMMARY_DIR / "day29_movies_medium20_2000_runtime_monitor.md").write_text("\n".join(lines), encoding="utf-8")
