...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input_path", type=str, default=None, help="Input ranking jsonl path.")
    ...
    parser.add_argument("--prompt_path", type=str, default="prompts/candidate_ranking.txt", help="Prompt template path.")
    parser.add_argument("--model_config", type=str, default=None, help="Model config path.")
    parser.add_argument("--task_type", type=str, default="candidate_ranking", help="Task type, fixed to candidate_ranking.")
...
def merge_config(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "exp_name": args.exp_name if args.exp_name is not None else cfg.get("exp_name", "ranking"),
        ...
        "prompt_path": args.prompt_path if args.prompt_path is not None else cfg.get("prompt_path", "prompts/candidate_ranking.txt"),
        "model_config": args.model_config if args.model_config is not None else cfg.get("model_config"),
        "task_type": args.task_type if args.task_type is not None else cfg.get("task_type", "candidate_ranking"),
...
def main() -> None:
    ...
    if task_type != "candidate_ranking":
        raise ValueError(f"main_rank.py only supports task_type=candidate_ranking, got: {task_type}")
    ...
    output_path = Path(output_path) if output_path is not None else predictions_dir / "rank_predictions.jsonl"
