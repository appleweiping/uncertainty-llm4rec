...
Week7 的服务器运行路线固定为 base-only first。当前主实验模型已经从 Llama 3.1 8B Instruct 切换为 Qwen3-8B，这是执行 backbone 的切换，不改变 week6 已经收敛出的 structured risk current best family，也不重新打开 ranking family 搜索。官方 API 后端保留为外部观察和小规模 cross-model 参考，不再承担主实验吞吐。
