...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--baseline_names", type=str, nargs="*", default=None)
...
def main() -> None:
    ...
    baseline_names = args.baseline_names or [args.baseline_name]
    status_rows = []
    for baseline_name in baseline_names:
        if baseline_name not in BASELINE_BUILDERS:
            raise ValueError(f"Unsupported minimal literature baseline: {baseline_name}")

        output_dir = Path(args.output_root) / "baselines" / "literature" / f"{args.exp_name}_{baseline_name}"
        prediction_dir = output_dir / "predictions"
        table_dir = output_dir / "tables"
        ensure_dir(prediction_dir)
        ensure_dir(table_dir)

        predictions = BASELINE_BUILDERS[baseline_name](samples, k=args.k)
