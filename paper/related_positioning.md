...
- **Sequential and graph baselines.** SASRec [SASRec], BERT4Rec [BERT4Rec], and LightGCN [LightGCN] are the intended reviewer-grade sequential and graph collaborative filtering comparators. The repository routes these through a RecBole adapter so that training can use the external library while final predictions are imported into the TRUCE schema and evaluated by the same evaluator as CU-GR v2. MovieLens 1M and Amazon Beauty now have TRUCE-evaluated SASRec, BERT4Rec, and LightGCN rows. The resulting framing should be complementary rather than dominance-based: CU-GR v2 is stronger than fallback and weak generative controls in the target gates, but MovieLens RecBole baselines slightly exceed CU-GR v2 by NDCG@10 under the current sampled-candidate table.
