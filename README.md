...
The current baseline-side validation line now includes first-batch minimal adapters for `CoVE` and `SLMRec`, plus a third extensible `LLM-ESR` shell. A separate `main_baseline_eval.py` entry runs the grouped candidate baseline evaluation path, and `main_baseline_summary.py` aggregates paper-facing baseline comparison tables under `outputs/baselines/summary/`.
