"""Day29 Movies medium_20neg_2000 calibration and SASRec plug-in analysis.

This script assumes inference is already complete. It does not call DeepSeek.
It runs valid-fit/test-evaluate calibration summaries and a Movies-only
SASRec-style external backbone plug-in with metric-repaired ranking metrics.
"""
...
EXP_NAME = "movies_deepseek_relevance_evidence_medium_20neg_2000"
OUTPUT_ROOT = "output-repaired"
SUMMARY_DIR = Path("output-repaired/summary")
BACKBONE_DIR = Path("output-repaired/backbone/sasrec_movies_medium20_2000")
TRAIN_PATH = Path("data/processed/amazon_movies_medium_20neg_2000/train.jsonl")
TEST_PATH = Path("data/processed/amazon_movies_medium_20neg_2000/test.jsonl")
...
def _ensure_complete() -> None:
    ...
    if not valid_path.exists() or not test_path.exists():
        raise FileNotFoundError("valid_raw.jsonl and test_raw.jsonl must both exist before Day29 analysis.")
    valid_rows = sum(1 for _ in valid_path.open("r", encoding="utf-8"))
    test_rows = sum(1 for _ in test_path.open("r", encoding="utf-8"))
    if valid_rows != 42000 or test_rows != 42000:
        raise RuntimeError(f"Expected valid/test rows 42000/42000, got {valid_rows}/{test_rows}.")
...
def _rank_metrics_repaired(df: pd.DataFrame, score_col: str) -> dict[str, float | bool]:
    ...
    def ndcg(k: int) -> float:
        if not ranks:
            return math.nan
        return float(np.mean([1.0 / math.log2(rank + 1) if rank <= k else 0.0 for rank in ranks]))
...
def _rerank_grid(joined: pd.DataFrame) -> pd.DataFrame:
    ...
    for normalization in ["minmax", "zscore"]:
        ...
        for lam in [0.0, 0.05, 0.1, 0.2, 0.5]:
            ...
            for alpha in [0.5, 0.75, 0.9]:
                beta = 1 - alpha
                score = alpha * norm_backbone + beta * norm_calibrated - lam * norm_risk
                settings.append(("D_SASRec_plus_calibrated_relevance_plus_evidence_risk", lam, alpha, beta, score))
...
def _write_report(valid_raw: pd.DataFrame, test_raw: pd.DataFrame, comparison: pd.DataFrame, diagnostics: pd.DataFrame, grid: pd.DataFrame, plugin_diag: pd.DataFrame) -> None:
    ...
    text = f"""# Day29 Movies medium_20neg_2000 Report

## 1. Why Movies medium_20neg_2000

Movies is the first regular-domain cross-domain validation because it uses the standardized medium-large split from regular processed data. The split has 2000 users, 21 candidates per user, and HR@10 is non-trivial.

## 2. Data Scale

Valid has `{len(valid_raw)}` rows and test has `{len(test_raw)}` rows. Each test user has 1 positive and 20 negatives.

## 3. API Inference

Valid parse_success is `{valid_raw['parse_success'].astype(bool).mean():.4f}` and test parse_success is `{test_raw['parse_success'].astype(bool).mean():.4f}`.

## 4. Relevance Calibration
...
## 7. Beauty Direction Check

If the best method is B or D and improves NDCG/MRR over SASRec-only, the direction is consistent with Beauty: calibrated relevance posterior is the main contributor, while evidence_risk is tested as a secondary regularizer.

## 8. Day30 Recommendation

If Movies SASRec is positive and stable, Day30 should either run Movies GRU4Rec/Bert4Rec on the same medium_20neg_2000 split or move to Books medium_20neg_2000 after one more Movies backbone confirmation.

## 9. Limitation

This is Movies medium-large, not full Movies. It is regular-domain derived and has candidate_pool_size=21, so HR@10 is meaningful, but it remains a medium benchmark.
"""
    (SUMMARY_DIR / "day29_movies_medium20_2000_report.md").write_text(text, encoding="utf-8")
...
def run_analysis() -> None:
    ...
    paths = ensure_exp_dirs(EXP_NAME, OUTPUT_ROOT)
    valid_raw = read_jsonl(paths.predictions_dir / "valid_raw.jsonl", "Day29 valid raw")
    test_raw = read_jsonl(paths.predictions_dir / "test_raw.jsonl", "Day29 test raw")
    comparison = read_csv(paths.tables_dir / "relevance_evidence_calibration_comparison.csv", "Day29 calibration comparison")
    comparison.to_csv(SUMMARY_DIR / "day29_movies_medium20_2000_calibration_comparison.csv", index=False)
    diagnostics = field_diagnostics(valid_raw, test_raw)
    diagnostics.to_csv(SUMMARY_DIR / "day29_movies_medium20_2000_field_diagnostics.csv", index=False)
    ...
    joined = _join_evidence(scores, evidence_path, SUMMARY_DIR / "day29_movies_sasrec_medium20_2000_joined_candidates.csv")
    join_diag = _join_diagnostics(joined, SUMMARY_DIR / "day29_movies_sasrec_medium20_2000_join_diagnostics.csv")
    grid = _rerank_grid(joined)
    grid.to_csv(SUMMARY_DIR / "day29_movies_sasrec_medium20_2000_plugin_rerank_grid.csv", index=False)
    plugin_diag = _plugin_diagnostics(joined, grid, join_diag)
    plugin_diag.to_csv(SUMMARY_DIR / "day29_movies_sasrec_medium20_2000_plugin_diagnostics.csv", index=False)
    _write_report(valid_raw, test_raw, comparison, diagnostics, grid, plugin_diag)
